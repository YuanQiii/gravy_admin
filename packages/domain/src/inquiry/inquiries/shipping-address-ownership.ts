...
/**
 * 收货地址的**归属不变量**：「地址存在 + 未软删 + 属于指定客户」三件事的唯一表达。
 *
 ...
 *
 * 三种失败共用同一错误码与消息（与引入本函数前的行为一致），因此不需要分支。
 *
 * 声明为**断言函数**（`asserts address is T`）而非返回布尔：调用方在调用之后即可
 * 安全使用 `address`，不需要再写一次空值判断 —— 否则"守卫通过"这件事只在运行时成立、
 * 在类型上丢失（本函数最初就因这一点导致 7 处 `address possibly null` 编译错误）。
 */
export function assertShippingAddressOwned<
  T extends { customerId: string; deletedAt: Date | null },
>(address: T | null, ownerCustomerId?: string | null): asserts address is T {
  if (
    !address ||
    address.deletedAt ||
    (ownerCustomerId && address.customerId !== ownerCustomerId)
  ) {
