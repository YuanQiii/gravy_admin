...
- 结构化日志内核为 `nestjs-pino`，收敛在深模块 `src/logging/`（见 ADR 0006）。生产输出单行 JSON，开发输出 pino-pretty 可读格式；级别由 `LOG_LEVEL` 控制，`pinoHttp.autologging` 已关闭。
...
- stdout 脱敏与 DB 审计脱敏为两条独立链路：pino `redact`（`SENSITIVE_KEYS` 单一来源 + `LOG_REDACT` 追加）负责 stdout；`OPLOG_MASK_FIELDS`（叠加 `SENSITIVE_KEYS`）负责落库。改日志/审计敏感字段名单时优先改 `src/shared/constants/sensitive-keys.constant.ts`。
