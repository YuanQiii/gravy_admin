...
- 常量位置：`packages/core/src/shared/constants/permissions.constant.ts`
...
- 真正公开的 GET 接口使用 `@Public()` 装饰器（`packages/core/src/core/decorators/public.decorator.ts`），`AccessGuard` 检测到 `IS_PUBLIC_KEY` 元数据后短路放行，不调用任何被编排守卫。
...
- 权限同步由 `POST /system/permissions/scan` 扫描 Controller 元数据完成（会新增/更新/删除权限记录）；**没有** 对应的 pnpm 脚本。
- 运行扫描会改变权限数据，执行前应明确确认。
