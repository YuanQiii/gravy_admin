...
本目录仅面向 AI 编程助手，作为按需知识库使用。根目录 [../../AGENTS.md](../../AGENTS.md) 是自动加载入口，应保持短小稳定；本目录文件不要默认全量读取。
...
- `.agents/project/`：AI 编程助手按需知识库，不是产品文档；保持短摘要，不复制根目录长文档。
- `docs/`：保留给未来产品/API/用户/部署文档。
- 根目录长文档：暂时保留，后续需要建设正式文档站时再迁移；更新时以源码/Swagger/OpenAPI 为准，避免维护易过期清单。
