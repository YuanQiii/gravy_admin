...
- [x] 6.2 compose 三份（dev/test/prod）改双 service：`gvray-admin-app`（entrypoint.sh → db-bootstrap → exec）与 `gvray-mall-app`（直接 exec CMD，不调 db-bootstrap）；entrypoint 行尾 LF（`git config core.autocrlf` 注意 + 容器实测 exit 0）；验证 `docker compose -f docker-compose.yml / docker-compose.dev.yml config -q` 通过、双 service 结构正确（dev-target 热重载 compose 运行另议，prod 镜像等效容器已实证）

- [x] 6.3 容器级冒烟：admin 容器 entrypoint `db-bootstrap`（migrationApplied=true）后 `/health` 200、`/api` 200、**customer token 打 admin** **`/equipment/filters`** **→ 401**（realm 断言 en vivo）；mall 容器 `/health` 200、匿名 `/filters` 与 `/equipment` 200（限流 60/min）、日志无 schema 同步、`operation_logs` 表匿名访问前后行数不变（mall 不落后台审计）
