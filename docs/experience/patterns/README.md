...
- **先例**：
  - 加权排序 → `runWeightedSort(prisma, spec)`（[weighted-sort.ts](../../packages/domain/src/equipment/weighted-sort.ts)），service 只留字段权重表/conditions/count thunk。
  - 热门排序 → `rankHotBrands(brands, deviceCounts, limit)`（[hot-ranking.ts](../../packages/domain/src/equipment/brands/hot-ranking.ts)），纯函数、零 prisma，排序不变量单点。
  ...
  - 结构化日志 → `packages/core/src/logging/`、数据库引导 → `packages/core/src/bootstrap/`（DI、可测，CLI 薄 adapter）。
...
- **先例**：[rankHotBrands](../../packages/domain/src/equipment/brands/hot-ranking.ts)、[runWeightedSort](../../packages/domain/src/equipment/weighted-sort.ts)。
...
- **先例**：[base.service.ts](../../packages/core/src/shared/services/base.service.ts) 的 `applyVisibility` / `isB2cVisibility`。
...
- **先例**：[access.guard.ts](../../packages/core/src/core/guards/access.guard.ts)、ADR 0005。
...
- **先例**：[session-store.service.ts](../../packages/core/src/core/session/session-store.service.ts) / [customer-token.service.ts](../../apps/mall/src/modules/customer-auth/customer-token.service.ts)。
...
- **先例**：[brand-response.dto.ts](../../packages/domain/src/equipment/brands/dto/brand-response.dto.ts)、[hot-brand-response.dto.ts](../../packages/domain/src/equipment/brands/dto/hot-brand-response.dto.ts)。
...
- **先例**：[permissions.constant.ts](../../packages/core/src/shared/constants/permissions.constant.ts)、[menus.ts](../../prisma/seeds/menus.ts)。